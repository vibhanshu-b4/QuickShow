...
const Mybookings = () => {
    ...
    }, [getToken, isLoaded, isSignedIn, openSignIn])
    useEffect(()=>{
        getMyBookings()
    },[getMyBookings])
  return !isLoading ?(
    <div className='relative px-6 md:px-16 lg:px-40 pt-30 md:pt-40 min-h-[80vh]'>
        ...
        {bookings.filter(item => item.show).map((item,index)=>(
            <div  key={index} className='flex flex-col md:flex-row justify-between bg-primary/8 border border-primary/20 rounded-lg mt-4 p-2 max-w-3xl'> 
                <div className='flex flex-col md:flex-row'>
                    <img src={item.show.movie?.poster_path} alt='' className='md:max-w-45 aspect-video h-auto objext-cover object-bottom rounded'/>
